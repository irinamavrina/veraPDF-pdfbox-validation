...
public class PBoxPDAnnot extends PBoxPDObject implements PDAnnot {
    ...
    public String getAlt() {
		...
		if (structTreeRoot != null && structParent != 0) {
			...
			PBoxPDNumberTreeNode treeNode = new PBoxPDNumberTreeNode(parentTreeRoot.getCOSObject(), PBoxPDParentTreeValue.class);
			COSBase structureElement = null;
			try {
				PBoxPDParentTreeValue o = (PBoxPDParentTreeValue) treeNode.getValue(structParent);
				structureElement = o.getCOSObject();
			} catch (IOException e) {
				e.printStackTrace();
			}
    ...
    public Boolean getisOutsideCropBox() {
        ...
        int structParent = ((PDAnnotation) this.simplePDObject).getStructParent();
        if (structTreeRoot != null && structParent != 0) {
            PBoxPDNumberTreeNode treeNode = new PBoxPDNumberTreeNode(structTreeRoot.getParentTree().getCOSObject(), PBoxPDParentTreeValue.class);
            try {
                PBoxPDParentTreeValue o = (PBoxPDParentTreeValue) treeNode.getValue(structParent);
                COSDictionary p = (COSDictionary) ((COSDictionary) o.getCOSObject()).getDictionaryObject(COSName.P);
                pdPage = p == null ? null : new PDPage(p);
            } catch (IOException e) {
                e.printStackTrace();
            }
